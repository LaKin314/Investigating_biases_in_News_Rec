...
def download_mind_large(download = False):
    ...
    if download:
        train_path,valid_path = download_mind(size='large', dest_path='/home/langenhagen/Masterthesis/Masterarbeit/Dataset_large')
    ...
    valid_clickhistory_large = read_clickhistory(path=f'/home/langenhagen/Masterthesis/Masterarbeit/MINDlarge_train.zip/valid',
                                        filename='behaviors.tsv')

    valid_clickhistory_small = read_clickhistory(path=f'/home/langenhagen/Masterthesis/Masterarbeit/Dataset_small/valid',
                                        filename='behaviors.tsv')
    train_clickhistory_small = read_clickhistory(path=f'/home/langenhagen/Masterthesis/Masterarbeit/Dataset_small/train',
                                        filename='behaviors.tsv')
...
def setup_full_mind():
    ...
    with open(os.path.join(f'/home/langenhagen/Masterthesis/Masterarbeit/MINDlarge_train.zip/valid',
                                    'behaviors.tsv')) as f:
        lines = f.readlines()
    sessions = []
    ...
    with open(os.path.join(f'/home/langenhagen/Masterthesis/Masterarbeit/Dataset_small/test/',
...
def get_dataframe(set_type : str, behav_file = True):
    ...
    if behav_file:
        # Path hardcoded 
        result_df = pd.read_csv(f"/home/langenhagen/Masterthesis/Masterarbeit/Dataset_small/{set_type}/behaviors.tsv",delimiter='\t')
        ...
        if set_type == "test":
            return None
        # Path hardcoded 
        result_df = pd.read_csv(f"/home/langenhagen/Masterthesis/Masterarbeit/MINDlarge_train.zip/{set_type}/news.tsv",delimiter='\t')
...
def get_test_news():
    large_valid = pd.read_csv("/home/langenhagen/Masterthesis/Masterarbeit/MINDlarge_train.zip/valid/news.tsv",sep='\t',header=None)
    ...
    large_train = pd.read_csv("/home/langenhagen/Masterthesis/Masterarbeit/MINDlarge_train.zip/train/news.tsv",sep='\t',header=None)
    ...
    small_train = pd.read_csv("/home/langenhagen/Masterthesis/Masterarbeit/Dataset_small/train/news.tsv",sep='\t',header=None)
    ...
    small_valid = pd.read_csv("/home/langenhagen/Masterthesis/Masterarbeit/Dataset_small/valid/news.tsv",sep='\t',header=None)
    ...
    filepath = Path('/home/langenhagen/Masterthesis/Masterarbeit/Dataset_small/test/news.tsv')
    vlarge.to_csv(filepath,sep='\t',index=False,header=False)

    return filepath

def behaviors_with_historysize(n : int,filepath = "/home/langenhagen/Masterthesis/Masterarbeit/Dataset_small/Users_with_n_behaviors",size_of_file = 156965,seed = 42):
    """Creates a new behaviors file with behaviors with a history of at least n 

    Args:
        n (int): Minimal history size 
        filepath (str, optional): Saving destination (without filename -> filepath/behaviors.tsv). Defaults to "/home/langenhagen/Masterthesis/Masterarbeit/Dataset_small/Users_with_n_behaviors".
    ...
    # !Hardcoded Paths
    if not os.path.exists("/home/langenhagen/Masterthesis/Masterarbeit/MINDlarge_train.zip"):
        download_mind_large(True)
    df_full = pd.read_csv("/home/langenhagen/Masterthesis/Masterarbeit/MINDlarge_train.zip/train/behaviors.tsv",sep="\t",names=['Impression ID', 'User ID', 'Time', 'History' , 'Impressions'])
    df_valid_small = pd.read_csv("/home/langenhagen/Masterthesis/Masterarbeit/MINDlarge_train.zip/train/behaviors.tsv",sep="\t",names=['Impression ID', 'User ID', 'Time', 'History' , 'Impressions'])
...
def main():
    extract_mind("/home/langenhagen/Masterthesis/Masterarbeit/Dataset_large/MINDlarge_train.zip",
        "/home/langenhagen/Masterthesis/Masterarbeit/Dataset_large/MINDlarge_dev.zip",
        train_folder="train",
        valid_folder="valid",
        clean_zip_file=True)
